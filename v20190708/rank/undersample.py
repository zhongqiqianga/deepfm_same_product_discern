...
#     click_data['pid'] = click_data['pid'].apply(md5_hash_trim)
#     train_data = pd.merge(product_data, click_data, on='pid')
...
def merge_userclick_product(userclick_file, product_file, user_file,output_file):
    ...
    print("Percentage of not click transactions: ",len(click_data[click_data.if_click == 0]))  # 打印正样本数目
    print("Percentage of click transactions: ",len(click_data[click_data.if_click == 1])) # 打印负样本数目
    ...
    number_click = len(click_data[click_data.if_click == 1])  # 计算出class这一列一号元素有多少个
    click_indices = np.array(click_data[click_data.if_click == 1].index)  # 取出class这一列所有等于1的行索引

    # Picking the indices of the normal classes
    not_click_indices = click_data[click_data.if_click == 0].index  # 取出class这一列所有等于0的行索引

    # Out of the indices we picked, randomly select "x" number (number_records_fraud)
    random_notclick_indices = np.random.choice(not_click_indices, 5*number_click,replace=False)  # 随机选择和1这个属性样本个数相同的0样本
    ...
    # Showing ratio
    print("Percentage of normal transactions: ",len(under_sample_data[under_sample_data.if_click == 0]) / len(under_sample_data))  # 打印正样本数目
    print("Percentage of fraud transactions: ",len(under_sample_data[under_sample_data.if_click == 1]) / len(under_sample_data))  # 打印负样本数目
...
if __name__ == "__main__":
    ...
    # merge_userclick_product(user_click_file,product_file_url,'')
    #     # if(len(sys.argv)<3):
    #     #     print("please input three para")
    #     # brand_index_file = sys.argv[1]
    #     # cate_index_file = sys.argv[2]
    #     # u_feature_file = sys.argv[3]
