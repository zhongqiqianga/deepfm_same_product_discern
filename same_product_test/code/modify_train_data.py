...
def mod_train_data(train_data, name_list, brand_list, cate_list, outfile_path):
    """
    根据name、brand、cate的相似度生成dataframe，并和原始数据进行拼接,将原始的一部分特征删除
    :param train_data:
    :param name_list:
    :return:
    """
...
if __name__ == "__main__":
    if (len(sys.argv) < 3):
        print("one of the file  not exist")
    else:
        raw_train_path = sys.argv[1]
        final_train_path = sys.argv[2]
        train_data_csv = gen_train_data(raw_train_path)
        name_list, brand_list, cate_list = gen_simi_list(train_data_csv)
        mod_train_data(train_data_csv, name_list, brand_list, cate_list, final_train_path)
