...
def get_binres(result_dict):
    for key in result_dict:
        temp_result=result_dict[key].copy()
        for ele in result_dict[key]:
            if(ele in result_dict):
                temp_result+=result_dict[ele]
        result_dict[key]=list(set(temp_result))
        if(key in result_dict[key]):
            result_dict[key].remove(key)
    return result_dict
...
if __name__=="__main__":
    result_url="/Users/looker/project/xmodel/same_product_judge/data/result_tran.csv"
    ...
    print(result_dict["p-nap-1137853"])
